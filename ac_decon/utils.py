...
def save_u_plots(opts, train_kl_u, validation_kl_u, mu_u_tr, mu_u_te, filename):

    dpi = 100
    pic_len = 28
    fig_height = 8 * pic_len / float(dpi)
    fig_width = 8 * 3 * pic_len / float(dpi)
    fig = plt.figure(figsize=(fig_width, fig_height))
    gs = matplotlib.gridspec.GridSpec(1, 3)

    total_num = len(train_kl_u)
    x = np.arange(1, total_num + 1)

    # img1
    plt.subplot(gs[0, 0])
    plt.plot(x, train_kl_u, linewidth=1, color='blue', label='train_kl_u')
    plt.plot(x, validation_kl_u, linewidth=1, color='green', label='validation_kl_u')
    plt.legend(loc='upper right')

    # img2
    plt.subplot(gs[0, 1])
    plt.plot(np.reshape(mu_u_tr[:, 0], opts['batch_size']), np.reshape(mu_u_tr[:, 1], opts['batch_size']),
             marker='o', color='blue', linestyle='None', label='mu_u_tr')
    plt.legend(loc='upper right')

    # img3
    plt.subplot(gs[0, 2])
    plt.plot(np.reshape(mu_u_te[:, 0], opts['batch_size']), np.reshape(mu_u_te[:, 1], opts['batch_size']),
             marker='o', color='green', linestyle='None', label='mu_u_te')
    plt.legend(loc='upper right')
...
def save_reward_plots(opts, r_list, filename):
    dpi = 100
    fig = plt.figure()
    gs = matplotlib.gridspec.GridSpec(1, 1)

    plt.subplot(gs[0, 0])
    plt.plot(np.arange(0, len(r_list)), r_list, linewidth=1, color='blue', label='reward_per_epoch')
    plt.legend(loc='upper right')
