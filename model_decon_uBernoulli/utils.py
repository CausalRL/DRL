...
def save_u_plots(opts, train_u_kl_divergence, validation_u_kl_divergence,
                 train_u_accuracy, validation_u_accuracy, filename):

    dpi = 100
    pic_len = 28
    fig_height = 8 * pic_len / float(dpi)
    fig_width = 8 * 2 * pic_len / float(dpi)
    fig = plt.figure(figsize=(fig_width, fig_height))
    gs = matplotlib.gridspec.GridSpec(1, 2)

    total_num = len(train_u_kl_divergence)
    x = np.arange(1, total_num + 1)

    # img1
    plt.subplot(gs[0, 0])
    plt.plot(x, train_u_kl_divergence, linewidth=1, color='blue', label='train_u_kl_divergence')
    plt.plot(x, validation_u_kl_divergence, linewidth=1, color='green', label='val_u_kl_divergence')
    plt.legend(loc='upper right')

    # img2
    plt.subplot(gs[0, 1])
    plt.plot(x, train_u_accuracy, linewidth=1, color='blue', label='train_u_acc')
    plt.plot(x, validation_u_accuracy, linewidth=1, color='green', label='val_u_acc')
    plt.legend(loc='upper right')
